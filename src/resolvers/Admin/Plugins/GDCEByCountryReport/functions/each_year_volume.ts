export function each_year_volume(imports_year, exports_year) {
  ...
  for (let i = 0; i < imports_year?.length; i++) {
    each_year_volume.push({
      ...
      volume: Number(imports_year[i]?.custom_value_usd + exports_year[i]?.custom_value_usd)
        ? Number(imports_year[i]?.custom_value_usd + exports_year[i]?.custom_value_usd)?.toFixed(2)
        : 0,
    });
  }
